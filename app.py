...
st.markdown("כלי לחישוב רמת הריחוק הממוצעת והצגת היישובים על המפה.")
...
if st.button("חשב רמת פיזור והצג מפה"):
    geolocator = Nominatim(user_agent="geo_dispersion_tool_v2")
    city_list = [c.strip() for c in input_cities.split(",") if c.strip()]
    
    locations = []
    map_data = [] # רשימה מיוחדת עבור המפה
    ...
    with st.spinner('מאתר מיקומים על המפה...'):
        for name in city_list:
            ...
            if loc:
                ...
                # Streamlit map צריכה פורמט של lat ו-lon
    ...
    if len(locations) < 2:
        ...
        # 2. תצוגת המפה (החלק החדש!)
        st.subheader("מפת פריסת היישובים")
        df_map = pd.DataFrame(map_data)
        st.map(df_map) # פקודה אחת שמייצרת מפה אינטראקטיבית
